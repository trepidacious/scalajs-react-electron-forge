...
object DownshiftDemo {
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def renderItem(item: String, index: Int, itemProps: js.Object, highlightedIndex: Option[Int], selectedItem: Option[String]): VdomElement = {
      val isHighlighted = highlightedIndex.contains(index)
      val isSelected = selectedItem.contains(item)

      //TODO make style prop available in MenuItem facade - for now
      //we can messily add it into the additional props
      itemProps.asInstanceOf[js.Dictionary[js.Any]]("style") = 
        js.Dynamic.literal(
          fontWeight = if (isSelected) 500 else 400
        )
      ...
      <.div(
        Downshift[String](
          onChange = (item: Option[String], c: RenderState[String]) => 
            scope.modState(_.copy(selectedItem = item)),
          ...
          (a: RenderState[String]) => {
            ...
            val inputProps = 
              a.getInputProps(js.Dynamic.literal(
                placeholder = "Search for a country"
              ))

            <.div(
              mui.TextField(fullWidth = true, InputProps = inputProps),
